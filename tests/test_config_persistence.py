...
from utils.config_persistence import load_config, save_config, CONFIG_FILE, DEFAULT_CONFIG

def test_load_config_returns_defaults_when_file_not_exists(tmp_path, monkeypatch):
    monkeypatch.setattr('utils.config_persistence.CONFIG_FILE', tmp_path / "nonexistent.json")
    config = load_config()
    assert config == DEFAULT_CONFIG

def test_save_and_load_config(tmp_path, monkeypatch):
    config_file = tmp_path / "config.json"
    monkeypatch.setattr('utils.config_persistence.CONFIG_FILE', config_file)
