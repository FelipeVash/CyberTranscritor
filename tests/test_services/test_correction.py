...
def test_correct_success():
    with patch('backend.services.correction_service.correct_text', return_value="text corrected") as mock:
...
def test_correct_error_handling():
    with patch('backend.services.correction_service.correct_text', side_effect=Exception("fail")):
        service = CorrectionService()
