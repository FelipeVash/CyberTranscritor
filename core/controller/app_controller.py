"""
Main application core.controller.
...
from core.backend.models.model_manager import ModelManager
...
from core.backend.dbus.dbus_service import DBusService
...
class AppController:
    ...
    def __init__(self):
        ...
        if not config_file_exists:
            from core.utils.hardware_detector import get_recommended_settings
    ...
    def correct_transcription(self):
        ...
        from core.frontend.dialogs import show_correction_dialog
    ...
    def get_gpu_memory_usage(self):
        """
        Retorna o uso de memória GPU como string (ex: "VRAM: 2.3/8.0 GB").
        Tenta rocm-smi (AMD), nvidia-smi (NVIDIA) e fallback para torch.
        """
        import subprocess
        import shutil
        import re
        ...
        # ------------------------------------------------------------
        # Método 1: rocm-smi (AMD) - formato específico retornado pelo seu sistema
        # ------------------------------------------------------------
        rocm_smi = shutil.which('rocm-smi')
        if rocm_smi:
            try:
                # Executa rocm-smi com showmeminfo vram
                ...
                if result.returncode == 0:
                    ...
                    # GPU[0]          : VRAM Total Used Memory (B): 1540362240
                    ...
                    if total_match and used_match:
                        ...
                        return f"VRAM: {used_gb:.1f}/{total_gb:.1f} GB"
            except Exception as e:
                logger.debug(f"rocm-smi parsing falhou: {e}")

        # ------------------------------------------------------------
        # Método 2: nvidia-smi (NVIDIA)
        # ------------------------------------------------------------
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    if len(parts) == 2:
                        ...
                        return f"VRAM: {used_mb/1024:.1f}/{total_mb/1024:.1f} GB"
            except Exception as e:
                logger.debug(f"nvidia-smi falhou: {e}")

        # ------------------------------------------------------------
        # Método 3: Fallback PyTorch (apenas memória alocada pelo PyTorch)
        # ------------------------------------------------------------
        try:
            import torch
            if torch.cuda.is_available():
                ...
                return f"VRAM: {allocated_gb:.1f}/{total_gb:.1f} GB (PyTorch)"
        except Exception as e:
            logger.debug(f"PyTorch CUDA falhou: {e}")
